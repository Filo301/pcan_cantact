...
MCU_LC   := $(shell echo $(MCU) | tr A-Z a-z)
BOARD_LC := $(shell echo $(BOARD) | tr A-Z a-z)

BUILD_DIR := build/$(BOARD_LC)/$(MCU_LC)
OUT_TAG   := $(BOARD_LC)_$(MCU_LC)
...
.PHONY: f042 f072
f042:
	$(MAKE) clean MCU=F042 BOARD=$(BOARD)
	$(MAKE) MCU=F042 BOARD=$(BOARD) $(BOARD)

f072:
	$(MAKE) clean MCU=F072 BOARD=$(BOARD)
